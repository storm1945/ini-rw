...
declare case_sensitive_sections=false
declare case_sensitive_keys=false
declare show_config_warnings=false
declare show_config_errors=false
...
function process_ini_file()
{
    ...
    while read -r line || [ -n "$line" ]; do                                   # no EOF after last line!! use and to protect 
        ...
        if [[ $line =~ ^"["(.+)"]"$ ]]; then                                   # Match pattern for a 'section'
            ...
            if [[ -z ${key} ]]; then
                ...
                eval "${section}_${key}_linenumber='${line_number}'"           # Use eval to declare a variable
            fi
        fi
    done < "$1"
}
...
# Set Value                                                                        #
# -------------------------------------------------------------------------------- #
# Set a value for a specific key from a named section.                        #
# -------------------------------------------------------------------------------- #
...
function set_value()
{
    ...
    for i in "${!keys[@]}"; do
        if [[ "${keys[$i]}" = "${key}" ]]; then
            eval linenumber='${'"${section}"'_'"${key}"'_linenumber}'
            sed -i -r ''"${linenumber}"'s/(=).*/\1'"${value}"'/' ${ini_filepath}
            eval "${section}_values[$i]=$value"
            return 0                                                               #if found then exit function
        fi
    done
    if [ ${#keys[@]} != 0 ]; then                                                  #if not found append new key-value pair to last line of designate section
        eval eval "local section_last_linenumber='$'{${section}_${keys[-1]}_linenumber}" 
        eval "sed -i \""${section_last_linenumber}a$key=$value"\" ${ini_filepath}"
        #sed -i '18a Old line' simple-example.conf
    fi                                                                                   
}
...
if [ $# == 3 ]
then
eval echo "$(get_value "$2" "$3")"
elif [ $# == 4 ]
then 
eval "set_value $2 $3 $4"
else
echo -e "Wrong parameter number.\nTO READ:sh ini-file-parser.sh [filepath] [section] [key]\nTO SET:sh ini-file-parser.sh [filepath] [section] [key] [value] " >&2
exit 1
fi
